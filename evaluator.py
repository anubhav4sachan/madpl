...
class DSTCSGDSEvaluator():
    def __init__(self, data_dir):
        ...
        self.cur_domain = ''
        self.complete_domain = []
        from config import DSTCSGDSConfig
        cfg = DSTCSGDSConfig(data_dir)
        self.belief_domains = cfg.belief_domains
        self.mapping = cfg.mapping
        # db = DBQuery(data_dir, cfg)
        # self.dbs = db.dbs
    ...
    def add_goal_dstc(self, goal):
        self.goal_dstc = deepcopy(goal)
        self.goal = {}
        doma = []
        for key in list(self.goal_dstc.keys()):
            if key != 'domain_ordering':
                doma.append(key)
        for doman in doma:
            self.goal[doman] = {}
        for doman in doma:
            if 'info' in list(self.goal_dstc[doman].keys()):
                for info_items in list(self.goal_dstc[doman]['info'].keys()):
                    self.goal[doman][info_items] = self.goal_dstc[doman]['info'][info_items]
            if 'reqt' in list(self.goal_dstc[doman].keys()):
                for reqt_items in self.goal_dstc[doman]['reqt']:
                    self.goal[doman][reqt_items] = '?'
        self.cur_domain = ''
        self.complete_domain = [] 
    ...
    def add_sys_da(self, da_turn):
        ...
        """                       
        for da_w_p in da_turn:
            domain, intent, slot = da_w_p.split('~')
            value = str(da_turn[da_w_p])
            da = '~'.join([domain, intent, slot])
            self.sys_da_array.append(da +'~' + value)
    ...
    def _inform_F1_goal_usr_dstc(self, goal, usr_history, domains=None):
        """
        judge if all the constraint/request information is expressed
        """
        if domains is None:
        ...
        # f = open(('dstc-data/dstc_services_allcaps.txt'), 'r')
        # content = f.read()
        # caps_domains = content.split('\n')[:-1]
        # f.close()
        ...
        for da in usr_history:
            ...
            if intent == 'inform':
                for it in domains:
                    if domain == str(it).lower():
                        inform_slot[it].add(slot)
            elif intent == 'request':
                for it in domains:
                    if domain == str(it).lower():
                        request_slot[it].add(slot)
        for domain in domains:
            for k, v in goal[domain].items():
                if v == '?':
                    if k in request_slot[domain]:
    ...
    def inform_F1(self, ref2goal=True, ansbysys=True, aggregate=True):
        ...
        if ansbysys:
            ...
            TP, FP, FN = self._inform_F1_goal_usr_dstc(goal, self.usr_da_array)
        if aggregate:
            try:
                rec = TP / (TP + FN)
            except ZeroDivisionError:
    ...
    def inform_F1_dstc(self, ref2goal=True, ansbysys=True, aggregate=True):
        if ref2goal:
            goal = self.goal
        else:
            goal = self._init_dict()
    ...
    def task_success(self, ref2goal=True):
        ...
        Task is successful if the final_goal_state.keys() has all the values
        """
    ...
    def domain_success_dstc(self, turn_data):
        """
        judge if the domain (subtask) is successfully completed
        """
