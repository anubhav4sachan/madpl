...
from utils import state_vectorize, state_vectorize_user, state_vectorize_dstc, state_vectorize_user_dstc
...
##########
##########
def sampler_dstc(pid, queue, evt, env, policy_usr, policy_sys, batchsz):
    """
    This is a sampler function, and it will be called by multiprocess.Process to sample data from environment by multiple
    processes.
    :param pid: process id
    ...
    while sampled_num < batchsz:
        ...
        for t in range(traj_len):
            # [s_dim_usr] => [a_dim_usr]
            s_vec = torch.Tensor(state_vectorize_user_dstc(s, env.cfg))
            a = policy_usr.select_action(s_vec.to(device=DEVICE)).cpu()
            ...
            # [s_dim] => [a_dim]
            next_s_vec = torch.Tensor(state_vectorize_dstc(next_s, env.cfg))
            next_a = policy_sys.select_action(next_s_vec.to(device=DEVICE)).cpu()
            ...
            if done:
                env.set_rollout(True)
                s_vec_next = torch.Tensor(state_vectorize_user_dstc(s, env.cfg))
                a_next = torch.zeros_like(a)
                next_s_next, _ = env.step_usr(s, a_next)
                next_s_vec_next = torch.Tensor(state_vectorize_dstc(next_s_next, env.cfg))
                env.set_rollout(False)
                ...
                if env.evaluator.cur_domain:
                    for slot, value in next_s['belief_state'][env.evaluator.cur_domain].items():
                        if value == '?':
                            for da in next_s['sys_action']:
                                d, i, k, p = da.split('~')
                                if i in ['INFORM', 'OFFER'] and k == slot:
                                    break
                            else:
                                # not answer request
                                r_sys -= 1
...
class DSTCLearner():
    def __init__(self, env_cls, args, cfg, process_num, infer=False):
        ...
        if not infer:
            ...
            self.policy_sys_optim = optim.AdamW(self.policy_sys.parameters(), lr=args.lr_policy)
            self.policy_usr_optim = optim.AdamW(self.policy_usr.parameters(), lr=args.lr_policy)
            self.vnet_optim = optim.Adamax(self.vnet.parameters(), lr=args.lr_vnet, weight_decay=args.weight_decay)
    ...
    def evaluate(self, N):
        ...
        for seed in range(N):
            ...
            print('goal', env.evaluator.goal)
            for t in range(traj_len):
                s_vec = torch.Tensor(state_vectorize_user_dstc(s, env.cfg)).to(device=DEVICE)
                # mode with policy during evaluation
                a = self.policy_usr.select_action(s_vec, False)
                next_s, done = env.step_usr(s, a)
                
                next_s_vec = torch.Tensor(state_vectorize_dstc(next_s, env.cfg)).to(device=DEVICE)
                next_a = self.policy_sys.select_action(next_s_vec, False)
                s = env.step_sys(next_s, next_a)
    ...
    def load(self, filename):
        ...
        if os.path.exists(best_pkl):
            ...
            # print(str(best_pkl) + " doesn't exists. Please pre-train the model first.")
            best = float('-inf')
        return best
    ...
    def sample(self, batchsz):
        ...
        for i in range(self.process_num):
            process_args = (i, queue, evt, self.env_list[i], self.policy_usr, self.policy_sys, process_batchsz)
            processes.append(mp.Process(target=sampler_dstc, args=process_args))
        for p in processes:
            # set the process as daemon, and it will be killed once the main process is stoped.
            p.daemon = True
            p.start()
        ...
        # we need to get the first Memory object and then merge others Memory use its append function.
        pid0, buff0 = queue.get()
        for gi in range(1, self.process_num):
            pid, buff_ = queue.get()
            buff0.append(buff_) # merge current Memory into buff0
        evt.set()
    ...
    def update(self, batchsz, epoch, best=None):
        ...
        if not backward:    # Backward is False
            reward = r_usr.mean().item() + r_sys.mean().item() + r_glo.mean().item()
            logging.debug('validation, epoch {}, reward {}'.format(epoch, reward))
            self.writer.add_scalar('train/reward', reward, epoch)
            if reward > best:
